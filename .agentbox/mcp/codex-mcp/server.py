...
"""Codex agent MCP server - exposes Codex to other agents."""
...
def _invoke_codex(task: str, timeout: Optional[int] = None) -> Dict[str, Any]:
    """Invoke Codex agent in one-shot mode.

    Args:
        task: Task prompt for Codex
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Codex task timed out after {timeout} seconds",
            "output": ""
        }
...
def _tool_spec() -> Dict[str, Any]:
    ...
    return {
        "name": "invoke_codex",
        "description": (
            "Invoke Codex agent to perform a task. "
            "Codex excels at code generation, implementation, and quick fixes. "
            "Use Codex for: writing code, implementing features, code validation, testing, refactoring. "
            "The invocation is one-shot: Codex executes the task autonomously and returns the result."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
def _handle_tools_call(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = _invoke_codex(task, timeout)
    ...
    if not result["success"]:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Codex invocation failed: {error_msg}"
                }
            ],
            "isError": True
        }

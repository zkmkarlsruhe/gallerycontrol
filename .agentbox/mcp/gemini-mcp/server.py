...
"""Gemini agent MCP server - exposes Gemini to other agents."""
...
def _invoke_gemini(task: str, timeout: Optional[int] = None) -> Dict[str, Any]:
    """Invoke Gemini agent in one-shot mode.

    Args:
        task: Task prompt for Gemini
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Gemini task timed out after {timeout} seconds",
            "output": ""
        }
...
def _tool_spec() -> Dict[str, Any]:
    ...
    return {
        "name": "invoke_gemini",
        "description": (
            "Invoke Gemini agent to perform a task. "
            "Gemini excels at research, comprehensive analysis, testing, and multimodal tasks. "
            "Use Gemini for: cross-validation, testing, thorough analysis, research, arbitration. "
            "The invocation is one-shot: Gemini executes the task autonomously and returns the result."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "task": {
                    "type": "string",
                    "description": "The task prompt to send to Gemini"
                },
                "timeout": {
                    "type": "integer",
...
def _handle_tools_call(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = _invoke_gemini(task, timeout)
    ...
    if not result["success"]:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"Gemini invocation failed: {error_msg}"
                }
            ],
            "isError": True
        }

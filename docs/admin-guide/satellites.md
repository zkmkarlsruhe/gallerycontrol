...
- Python 3.11+
- Network access to GalleryControl server (HTTPS/WSS)
...
```yaml
server:
  url: wss://gallerycontrol.museum.local/ws/satellite
...
Create `/etc/systemd/system/gallerycontrol-satellite.service`:

```ini
[Unit]
Description=GalleryControl Satellite Daemon
...
```bash
sudo systemctl enable gallerycontrol-satellite
sudo systemctl start gallerycontrol-satellite
```
...
2. It will connect to GalleryControl and appear as "Pending"

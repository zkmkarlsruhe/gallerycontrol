...
The satellite relay system allows GalleryControl to manage devices on remote or isolated networks.

## Overview

Satellites are small daemons that run on remote networks and relay device commands from the central GalleryControl server. This is useful when:
...
```
┌─────────────────────────────┐     ┌──────────────────────────────┐
│   GalleryControl Server     │     │   Remote Network             │
│                             │     │                              │
│  ┌───────────────────────┐  │     │  ┌────────────────────────┐  │
...
**Connection Flow:**
1. Satellite connects to the GalleryControl server via Server-Sent Events (SSE)
...
- Outbound HTTPS connection from satellite to GalleryControl server

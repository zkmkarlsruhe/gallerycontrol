...
class TestParser:
    ...
    SAMPLE_RESPONSE = "NET-PwrCtrl:NET-CONTROL:192.168.232.95:255.255.252.0:192.168.232.1:0.4.163.17.8.92:Diaprojektoren,0:Dose 2,0:Dose 3,0:Nr. 4,0:Nr. 5,0:Nr. 6,0:Nr. 7,0:Nr. 8,0:248:80:NET-PWRCTRL_04.5:H:xo"
    ...
    def test_parse_status_response_valid(self):
        ...
        assert status.ip == "192.168.232.95"
        assert status.name == "NET-CONTROL"
        assert status.mac == "0.4.163.17.8.92"
    ...
    def test_parse_status_response_ports(self):
        ...
        assert status.ports[0].name == "Diaprojektoren"
        assert status.ports[0].state == 0

        assert status.ports[1].port == 1
        assert status.ports[1].name == "Dose 2"
